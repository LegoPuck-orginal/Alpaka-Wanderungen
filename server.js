...
app.post('/api/gutschein-bestellung', async (req, res) => {
  try {
    const gutscheinData = req.body;
    
    // Validierung
    if (!gutscheinData.kaeuferVorname || !gutscheinData.kaeuferNachname || !gutscheinData.kaeuferEmail) {
      return res.status(400).json({ error: 'Käufer-Daten sind unvollständig' });
    }
    
    if (!gutscheinData.versandStrasse || !gutscheinData.versandPlz || !gutscheinData.versandOrt) {
      return res.status(400).json({ error: 'Versand-Adresse ist unvollständig' });
    }
    
    // Gutschein-Bestellung erstellen
    const gutscheinBestellung = {
      id: generateId(),
      typ: 'gutschein-bestellung',
      
      // Käufer-Daten
      kaeufer: {
        vorname: gutscheinData.kaeuferVorname,
        nachname: gutscheinData.kaeuferNachname,
        email: gutscheinData.kaeuferEmail,
        telefon: gutscheinData.kaeuferTelefon || ''
      },
      
      // Empfänger-Daten
      empfaenger: gutscheinData.empfaengerTyp === 'geschenk' ? {
        vorname: gutscheinData.empfaengerVorname || '',
        nachname: gutscheinData.empfaengerNachname || '',
        email: gutscheinData.empfaengerEmail || ''
      } : {
        vorname: gutscheinData.kaeuferVorname,
        nachname: gutscheinData.kaeuferNachname,
        email: gutscheinData.kaeuferEmail
      ...
      // Versand-Adresse
      versandAdresse: {
        strasse: gutscheinData.versandStrasse,
        plz: gutscheinData.versandPlz,
        ort: gutscheinData.versandOrt
      },
      
      // Gutschein-Details
      gutschein: {
        typ: gutscheinData.gutscheinTyp,
        betrag: parseFloat(gutscheinData.finalerBetrag),
        beschreibung: gutscheinData.beschreibung,
        tour: gutscheinData.gutscheinTour || null,
        nachricht: gutscheinData.gutscheinNachricht || '',
        besondereWuensche: gutscheinData.besondereWuensche || ''
      },
      ...
      versandstatus: 'nicht_versandt',
      gutscheinCode: null // Wird später generiert
    };
    
    // In Bookings speichern (da es eine Bestellung ist)
    const bookings = await readJsonFile('bookings.json');
    bookings.push(gutscheinBestellung);
    await writeJsonFile('bookings.json', bookings);
    
    console.log(`🎁 Neue Gutschein-Bestellung erhalten: ${gutscheinBestellung.id}`);
    console.log(`📧 Käufer: ${gutscheinData.kaeuferVorname} ${gutscheinData.kaeuferNachname}`);
    console.log(`💰 Betrag: ${gutscheinData.finalerBetrag}€`);
    ...
    if (transporter) {
      try {
        ...
        const kundenMailOptions = {
          from: process.env.EMAIL_USER,
          to: gutscheinData.kaeuferEmail,
          subject: '🎁 Ihre Gutschein-Bestellung bei Alpaka-Wanderungen',
          html: `
            <h2>🦙 Vielen Dank für Ihre Gutschein-Bestellung!</h2>
            <p>Liebe/r ${gutscheinData.kaeuferVorname} ${gutscheinData.kaeuferNachname},</p>
            
            <p>wir haben Ihre Gutschein-Bestellung erhalten und bearbeiten diese schnellstmöglich.</p>
            
            <h3>📝 Ihre Bestellung:</h3>
            <ul>
              <li><strong>Gutschein:</strong> ${gutscheinData.beschreibung}</li>
              <li><strong>Betrag:</strong> ${gutscheinData.finalerBetrag}€</li>
              <li><strong>Empfänger:</strong> ${gutscheinData.empfaengerTyp === 'geschenk' ? 
                `${gutscheinData.empfaengerVorname} ${gutscheinData.empfaengerNachname}` : 
                'Sie selbst'}</li>
              <li><strong>Bestellnummer:</strong> ${gutscheinBestellung.id}</li>
            </ul>
            
            ${gutscheinData.gutscheinNachricht ? `
            <h3>💌 Ihre persönliche Nachricht:</h3>
            <p style="font-style: italic;">"${gutscheinData.gutscheinNachricht}"</p>
            ` : ''}
            
            <h3>📦 Nächste Schritte:</h3>
            <ol>
              <li>Sie erhalten in Kürze eine separate E-Mail mit den Zahlungsinformationen</li>
              <li>Nach Zahlungseingang erstellen wir Ihren personalisierten Gutschein</li>
              <li>Der Gutschein wird an die angegebene Adresse versandt</li>
            </ol>
            
            <p>Bei Fragen können Sie uns jederzeit kontaktieren.</p>
            
            <p>Herzliche Grüße<br>
            Ihr Team von Alpaka-Wanderungen 🦙</p>
          `
        };
        
        await transporter.sendMail(kundenMailOptions);
        
        // E-Mail an Admin
        const adminMailOptions = {
          from: process.env.EMAIL_USER,
          to: process.env.EMAIL_USER,
          subject: `🎁 Neue Gutschein-Bestellung: ${gutscheinData.finalerBetrag}€`,
          html: `
            <h2>🎁 Neue Gutschein-Bestellung eingegangen</h2>
            
            <h3>👤 Käufer:</h3>
            <ul>
              <li><strong>Name:</strong> ${gutscheinData.kaeuferVorname} ${gutscheinData.kaeuferNachname}</li>
              <li><strong>E-Mail:</strong> ${gutscheinData.kaeuferEmail}</li>
              <li><strong>Telefon:</strong> ${gutscheinData.kaeuferTelefon || 'Nicht angegeben'}</li>
            </ul>
            
            <h3>🎯 Empfänger:</h3>
            <ul>
              <li><strong>Typ:</strong> ${gutscheinData.empfaengerTyp === 'geschenk' ? 'Geschenk' : 'Für sich selbst'}</li>
              ${gutscheinData.empfaengerTyp === 'geschenk' ? `
              <li><strong>Name:</strong> ${gutscheinData.empfaengerVorname} ${gutscheinData.empfaengerNachname}</li>
              <li><strong>E-Mail:</strong> ${gutscheinData.empfaengerEmail || 'Nicht angegeben'}</li>
              ` : ''}
            </ul>
            
            <h3>🎁 Gutschein-Details:</h3>
            <ul>
              <li><strong>Typ:</strong> ${gutscheinData.gutscheinTyp}</li>
              <li><strong>Beschreibung:</strong> ${gutscheinData.beschreibung}</li>
              <li><strong>Betrag:</strong> ${gutscheinData.finalerBetrag}€</li>
              ${gutscheinData.gutscheinNachricht ? `<li><strong>Nachricht:</strong> "${gutscheinData.gutscheinNachricht}"</li>` : ''}
            </ul>
            
            <h3>📦 Versand-Adresse:</h3>
            <ul>
              <li><strong>Straße:</strong> ${gutscheinData.versandStrasse}</li>
              <li><strong>PLZ/Ort:</strong> ${gutscheinData.versandPlz} ${gutscheinData.versandOrt}</li>
            </ul>
            
            ${gutscheinData.besondereWuensche ? `
            <h3>✨ Besondere Wünsche:</h3>
            <p>${gutscheinData.besondereWuensche}</p>
            ` : ''}
            
            <p><strong>Bestellnummer:</strong> ${gutscheinBestellung.id}</p>
            <p><strong>Bestellzeit:</strong> ${new Date().toLocaleString('de-DE')}</p>
            
            <p>💼 Bitte im Admin-Panel bearbeiten: <a href="http://localhost:${PORT}/admin">Admin-Panel öffnen</a></p>
          `
        };
        
        await transporter.sendMail(adminMailOptions);
        console.log('📧 E-Mails für Gutschein-Bestellung versendet');
        
      } catch (emailError) {
        console.error('⚠️ E-Mail-Versand fehlgeschlagen:', emailError.message);
        // Nicht den ganzen Request fehlschlagen lassen
      }
    }
    
    res.status(201).json({ 
      success: true, 
      message: 'Gutschein-Bestellung erfolgreich eingegangen',
      bestellnummer: gutscheinBestellung.id
    });
    
  } catch (error) {
    console.error('❌ Fehler bei Gutschein-Bestellung:', error);
    res.status(500).json({ error: 'Server-Fehler beim Verarbeiten der Bestellung' });
  }
});
